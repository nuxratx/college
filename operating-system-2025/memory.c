...
struct MEMORY_BLOCK best_fit_allocate(int request_size, struct MEMORY_BLOCK memory_map[MAPMAX], int *map_cnt, int process_id) {
    ...
    if (match == true) {
        if (request_size < memory_map[memory_map_index].segment_size) {
            ...
            allocated_memory.start_address = memory_map[memory_map_index].start_address;
            allocated_memory.end_address = memory_map[memory_map_index].start_address + request_size - 1;
            ...
            *map_cnt = *map_cnt + 1;
            ...
            memory_map[memory_map_index+1].segment_size = memory_map[memory_map_index].segment_size - allocated_memory.segment_size;
...
struct MEMORY_BLOCK next_fit_allocate(int request_size, struct MEMORY_BLOCK memory_map[MAPMAX], int *map_cnt, int process_id, int last_address) {
    ...
    for (int i = 0; i<*map_cnt; i++) {
        if ((memory_map[i].segment_size >= request_size) && (memory_map[i].process_id == 0) && (memory_map[i].start_address >= last_address)) {
            if (match == false) {
                memory_map_index = i;
                best_fit_segment = memory_map[i].segment_size;
                match = true;
